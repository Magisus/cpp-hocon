...
namespace hocon { namespace default_transformer {
    ...
    shared_value transform(shared_value value, config_value_type requested) {
        if (value->value_type() == config_value_type::STRING) {
            ...
            switch (requested) {
                case config_value_type::NUMBER:
                    ...
                    break;
                case config_value_type::CONFIG_NULL:
                    if (s == "null") {
                ...
                case config_value_type::STRING:
                    ...
                    break;
                case config_value_type::UNSPECIFIED:
                    throw config_exception("No target value type specified");
            }
        } else if (requested == config_value_type::STRING) {
        ...
        } else if (requested == config_value_type::LIST && value->value_type() == config_value_type::OBJECT) {
