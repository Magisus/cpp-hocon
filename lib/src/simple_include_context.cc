...
namespace hocon {
    ...
    shared_ptr<simple_include_context> simple_include_context::with_parseable(
            shared_ptr<parseable> new_parseable) {
        return make_shared<simple_include_context>(new_parseable);
    }

    std::shared_ptr<config_parseable> simple_include_context::relative_to(std::string file_name) {
        if (_parseable) {
    ...
    config_parse_options simple_include_context::parse_options() {
